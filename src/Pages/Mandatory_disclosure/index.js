import React from 'react';

const Mandatory_disclosure = () => {
    return (
        <>
            <div className="container-fluid py-5">
                <div className="container">
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="mas_h">Mandatory Public Disclosure</div>
                            <br />

                            <div>
                                <div
                                    className="col-xl-12 col-md-12 col-lg-12 animate-fadeInRight"
                                    style={{ animationDuration: "1.5s" }}
                                >
                                    {/* A - GENERAL INFORMATION */}
                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td colSpan="3">
                                                    <strong>A• GENERAL INFORMATION :-</strong>
                                                </td>
                                            </tr>
                                            <tr className="clr-dark-blue">
                                                <td><strong>SL NO.</strong></td>
                                                <td><strong>INFORMATION</strong></td>
                                                <td><strong>DETAILS</strong></td>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue"><strong>1.</strong></td>
                                                <td>NAME OF THE SCHOOL</td>
                                                <td><strong>GAYATRI GURUKUL PUBLIC SCHOOL</strong></td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>2.</strong></td>
                                                <td>AFFILIATION NO.(IF APPLICABLE)</td>
                                                <td><strong>-</strong></td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>3.</strong></td>
                                                <td>SCHOOL CODE (IF APPLICABLE)</td>
                                                <td><strong>-</strong></td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>4.</strong></td>
                                                <td>COMPLETE ADDRESS WITH PIN CODE</td>
                                                <td>
                                                    <strong>
                                                        SARAYKALIDAS, MARIAHU, JAUNPUR, UTTARPRADESH - 222161
                                                    </strong>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>5.</strong></td>
                                                <td>PRINCIPAL NAME</td>
                                                <td><strong>MRS. NEETA VISHWAKARMA</strong></td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>6.</strong></td>
                                                <td>PRINCIPAL QUALIFICATION</td>
                                                <td><strong>M.A., B.ED, DNYS</strong></td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>7.</strong></td>
                                                <td>SCHOOL EMAIL ID</td>
                                                <td><strong>gayatriyoga666@gmail.com</strong></td>
                                            </tr>
                                            <tr>
                                                <td className="clr-light-blue"><strong>8.</strong></td>
                                                <td>CONTACT DETAILS (LANDLINE/MOBILE)</td>
                                                <td><strong>+91 9125603155</strong></td>
                                            </tr>
                                        </tbody>
                                    </table>

                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td colSpan="3">
                                                    <strong>B• DOCUMENTS AND INFORMATION :-</strong>
                                                </td>
                                            </tr>
                                            <tr className="clr-dark-blue">
                                                <td><strong>SL NO.</strong></td>
                                                <td><strong>DOCUMENTS/INFORMATION</strong></td>
                                                <td><strong>UPLOAD DOCUMENTS</strong></td>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue"><strong>1.</strong></td>
                                                <td>
                                                    COPIES OF AFFILIATION / UPGRADATION LETTER AND RECENT EXTENSION OF
                                                    AFFILIATION, IF ANY
                                                </td>
                                                <td>
                                                    <a href="/" target="_blank" rel="noreferrer">Click Here</a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>2.</strong></td>
                                                <td>
                                                    COPIES OF SOCIETIES / TRUST / COMPANY REGISTRATION / RENEWAL
                                                    CERTIFICATE AS APPLICABLE
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/B 2 COPIES OF TRUST CERTIFICATE.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>3.</strong></td>
                                                <td>
                                                    COPY OF NO OBJECTION CERTIFICATE (NOC) ISSUED, IF APPLICABLE BY
                                                    THE STATE GOVT / UT
                                                </td>
                                                <td>
                                                    <a href="/" target="_blank" rel="noreferrer">Click Here</a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>4.</strong></td>
                                                <td>
                                                    COPIES OF RECOGNITION CERTIFICATE UNDER RTE ACT. 2009. AND ITS
                                                    RENEWAL F APPLICABLE
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/B 4 COPIES OF RECOGNITION CERTIFICATE.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>5.</strong></td>
                                                <td>
                                                    COPY OF VALID BUILDING SAFETY CERTIFICATE AS PER THE NATIONAL
                                                    BUILDING CODE
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/B 5 COPY OF VALID BUILDING SAFETY CERTIFICATE.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>6.</strong></td>
                                                <td>
                                                    COPY OF VALID FIRE SAFETY CERTIFICATE ISSUED BY THE COMPETENT
                                                    AUTHORITY
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/B 6 COPY OF VALID FIRE SAFETY CERTIFICATE.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>7.</strong></td>
                                                <td>
                                                    COPY OF THE DEO CERTIFICATE SUBMITTED BY THE SCHOOL FOR
                                                    AFFILIATION / UPGRADATION / EXTENSION OF AFFILIATION OR SELF
                                                    CERTIFICATION BY SCHOOL
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/B 7 COPY OF THE SELF CERTIFICATION.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>8.</strong></td>
                                                <td>
                                                    COPIES OF VALID WATER HEALTH AND SANITATION CERTIFICATES
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/B 8 COPIES OF VALID WATER HEALTH AND SANITATION.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>9.</strong></td>
                                                <td>COPIES OF LAND CERTIFICATE</td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/Gayatri Land Certificate.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>


                                    {/* NOTE */}
                                    <p style={{ textAlign: "justify" }}>
                                        <strong>Note:-</strong> THE SCHOOL NEEDS TO UPLOAD THE SELF
                                        ATTESTED COPIES OF ABOVE LISTED DOCUMENTS BY
                                        CHAIRMAN/MANAGER/SECRETARY AND PRINCIPAL. IN CASE IT IS
                                        NOTICED AT LATER STAGE THAT UPLOADED DOCUMENTS ARE NOT
                                        GENUINE THEN SCHOOL SHALL BE LIABLE FOR ACTION AS PER NORMS.
                                    </p>


                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td colSpan="3">
                                                    <strong>C• RESULT AND ACADEMICS :-</strong>
                                                </td>
                                            </tr>
                                            <tr className="clr-dark-blue">
                                                <td><strong>SL NO.</strong></td>
                                                <td><strong>DOCUMENTS/INFORMATION</strong></td>
                                                <td><strong>UPLOAD DOCUMENTS</strong></td>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue"><strong>1.</strong></td>
                                                <td>FEE STRUCTURE OF THE SCHOOL</td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/C 1 FEE STRUCTURE OF THE SCHOOL.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>2.</strong></td>
                                                <td>ANNUAL ACADEMIC CALANDER</td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/C 2 ANNUAL ACADEMIC CALANDER.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>3.</strong></td>
                                                <td>LIST OF SCHOOL MANAGEMENT COMMITTEE (SMC)</td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/C 3 LIST OF SCHOOL MANAGEMENT COMMITTEE (SMC).pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>4.</strong></td>
                                                <td>LIST OF PARENTS TEACHERS ASSOCIATION (PTA) MEMBERS</td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/C 4 LIST OF PARENTS TEACHERS ASSOCIATION (PTA) MEMBERS.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>5.</strong></td>
                                                <td>
                                                    LAST THREE-YEAR RESULT OF THE BOARD EXAMINATION AS PER
                                                    APPLICABLITY
                                                </td>
                                                <td>
                                                    <strong>Not Applicable</strong>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>6.</strong></td>
                                                <td>
                                                    ALL HARASSMENT COMMITTEE (POCSO, POSH, SEXUAL HARASSMENT ETC)
                                                </td>
                                                <td>
                                                    <a
                                                        href="/imp_doc_school/C 6 All Harassment Committee (POCSO, POSH, SEXUAL HARASSMENT ETC).pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        Click Here
                                                    </a>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td colSpan="3">
                                                    <strong>D• STAFF (TEACHING) :-</strong>
                                                </td>
                                            </tr>
                                            <tr className="clr-dark-blue">
                                                <td><strong>SL NO.</strong></td>
                                                <td><strong>INFORMATION</strong></td>
                                                <td><strong>DETAILS</strong></td>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue"><strong>1.</strong></td>
                                                <td>PRINCIPAL</td>
                                                <td><strong>MRS. NEETA VISHWAKARMA</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>2.</strong></td>
                                                <td>TOTAL NO. OF TEACHERS</td>
                                                <td><strong>19</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong></strong></td>
                                                <td>PGT</td>
                                                <td><strong>0</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong></strong></td>
                                                <td>TGT</td>
                                                <td><strong>6</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong></strong></td>
                                                <td>PRT</td>
                                                <td><strong>8</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>3.</strong></td>
                                                <td>TEACHERS SECTION RATIO</td>
                                                <td><strong>1:1.5</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>4.</strong></td>
                                                <td>DETAILS OF SPECIAL EDUCATOR</td>
                                                <td><strong>1</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>5.</strong></td>
                                                <td>DETAILS OF COUNSELLOR AND WELNESS TEACHER</td>
                                                <td><strong>1</strong></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td className="text-center" colSpan="6">
                                                    <p>
                                                        <strong>RESULT CLASS: X</strong>
                                                    </p>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>SL NO.</strong></td>
                                                <td><strong>YEAR</strong></td>
                                                <td>
                                                    <strong>
                                                        NO. OF REGISTERED<br />STUDENTS
                                                    </strong>
                                                </td>
                                                <td>
                                                    <strong>
                                                        NO. OF STUDETNS<br />PASSED
                                                    </strong>
                                                </td>
                                                <td><strong>PASS PERCENTAGE</strong></td>
                                                <td><strong>REMARKS</strong></td>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue">1.</td>
                                                <td>{""}</td>
                                                <td><strong>{""}</strong></td>
                                                <td>{""}</td>
                                                <td>{""}</td>
                                                <td>{""}</td>
                                            </tr>
                                        </tbody>
                                    </table>


                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td className="text-center" colSpan="6">
                                                    <p>
                                                        <strong>RESULT CLASS: XII</strong>
                                                    </p>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>SL NO.</strong></td>
                                                <td><strong>YEAR</strong></td>
                                                <td>
                                                    <strong>
                                                        NO. OF REGISTERED<br />STUDENTS
                                                    </strong>
                                                </td>
                                                <td>
                                                    <strong>
                                                        NO. OF STUDETNS<br />PASSED
                                                    </strong>
                                                </td>
                                                <td><strong>PASS PERCENTAGE</strong></td>
                                                <td><strong>REMARKS</strong></td>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue">1.</td>
                                                <td>{""}</td>
                                                <td><strong>{""}</strong></td>
                                                <td>{""}</td>
                                                <td>{""}</td>
                                                <td>{""}</td>
                                            </tr>
                                        </tbody>
                                    </table>

                                    <table className="table table-hover table-bordered" width="100%">
                                        <thead>
                                            <tr className="info">
                                                <td colSpan="3">
                                                    <strong>E• SCHOOL INFRASTRUCTURE :-</strong>
                                                </td>
                                            </tr>

                                            <tr className="clr-dark-blue">
                                                <td><strong>SL NO.</strong></td>
                                                <td><strong>INFORMATION</strong></td>
                                                <td><strong>DETAILS</strong></td>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr>
                                                <td className="clr-light-blue"><strong>1.</strong></td>
                                                <td>TOTAL CAMPUS AREA OF THE SCHOOL (IN SQ MTR)</td>
                                                <td><strong>6126</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>2.</strong></td>
                                                <td>NO AND SIZE OF THE CLASS ROOMS (IN SQ MTR)</td>
                                                <td><strong>15 &amp; 47</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>3.</strong></td>
                                                <td>NO. AND SIZE OF LABORATORIES INCLUDING COMPUTER LABS (IN SQ MTR)</td>
                                                <td><strong>3 &amp; 56</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>4.</strong></td>
                                                <td>INTERNET FACILITY (Y/N)</td>
                                                <td><strong>YES</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>5.</strong></td>
                                                <td>NO. OF GIRLS TOILETS</td>
                                                <td><strong>4</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>6.</strong></td>
                                                <td>NO. OF BOYS TOILETS</td>
                                                <td><strong>4</strong></td>
                                            </tr>

                                            <tr>
                                                <td className="clr-light-blue"><strong>7.</strong></td>
                                                <td>
                                                    LINK OF YOUTUBE VIDEO OF THE INSPECTION OF SCHOOL COVERING THE
                                                    INFRASTRUCTURE OF THE SCHOOL
                                                </td>
                                                <td>
                                                    <strong>
                                                        <a
                                                            href="https://www.youtube.com/watch?v=Z_K7Nk4KXek"
                                                            target="_blank"
                                                            rel="noreferrer"
                                                        >
                                                            Video
                                                        </a>
                                                    </strong>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>


                                    {/* SARAS LINK */}
                                    <table className="table table-hover table-bordered" width="100%">
                                        <tbody>
                                            <tr className="info">
                                                <td colSpan="3" style={{ textAlign: "center" }}>
                                                    <a
                                                        href="/imp_doc_school/Mandatory Disclosure Details _ SARAS 6.0.pdf"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                    >
                                                        <span style={{ color: "blue" }}>
                                                            <strong>SARAS 6.0 - Mandatory Public Disclosure</strong>
                                                        </span>
                                                    </a>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>

                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </>
    );
};




export default Mandatory_disclosure;
